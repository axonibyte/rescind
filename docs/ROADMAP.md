# rescind — roadmap and plan of record

**A language for provably reversible operations.**

> Undo as a compiler verdict. A rescind plan is a sequence of operations whose
> reversibility — how far it can be undone, from where, by whom, and past
> which point it cannot — is decided by `rescind check` before anything runs,
...
2. §1 — the claim and its prior art. Know what rescind is *not* before writing it.
...
### 0.2 Engineering discipline (binding on all rescind code)
...
- **Pure core, injected time.** `rescind-core` performs no I/O and reads no clock. Every observation takes `now`. Expiry is a property of observation, not of a background thread.
...
### 0.3 What rescind is not (scope fence)

- **Not a configuration-management or convergence tool.** Terraform, Ansible and Kubernetes converge toward declared state. Rescind sequences operations and proves their reversal. Where convergence is the right model, rescind is the wrong tool, and the README says so.
...
- **Not a secrets manager.** Secrets are a binding (§7.3). Rescind holds a one-time value only long enough to deliver it once.
- **Not an inventory system.** Inventory is a binding. Rescind defines the contract a host record must satisfy and nothing about where it lives.
- **Not a state-machine language.** Reactive rules and control loops belong to the host (T4, §8.4). A machine decides *when* to run a plan; rescind decides *how* and *how to take it back*.
- **Not a replication, detection, fencing, authority or elevation engine.** Those are ops, probes and bindings a tenant declares; rescind calls them.

### 0.4 Capability, not reimplementation (the tenant test)

Rescind was distilled from four projects. The recurring design mistake was to lift one of their *policies* into the engine. The rule that prevents it, applied to every mechanism in this document and to every mechanism proposed later:

> Rescind provides a mechanism itself only if (a) the checker needs it in order to state a verdict, or (b) it carries a guarantee rescind makes that cannot be delegated without handing the guarantee to the tenant — write-ahead journaling, reveal-once, the chain, footprint drift detection. Everything else is a **binding with a contract**, declared in the language, and the engine ships at most one generic built-in per binding kind.

| Mechanism | Verdict | Why |
|---|---|---|
| Gate *shape* (threshold grammar) | rescind | The checker needs it to say "unsatisfiable" and "N humans minimum" |
| Challenge rendering, proof verification, key material, approval UX | binding | Trust and identity are the tenant's; rescind supplies a request digest and consumes "authenticator X satisfied" |
| Renewal windows, TTL caps, approval windows | declared | Policy numbers are site/plan declarations; the engine has no defaults |
| Rendering a `:target` undo into a standalone artifact | rescind | It is what `undo_locus: :target` *means* |
| The scheduler that runs that artifact | binding | Platform vocabulary; generic built-ins `cron()` and `task_scheduler()` only |
| Secret delivery (requester, hold, escrow) | binding | Where a credential goes is credential policy; rescind guarantees once-and-nowhere-else |
| Elevation (sudo/doas/runas) | not in v0 | Rescind has no elevation; `rescind bootstrap` verifies and prints, never runs |
| Instance lifecycle, stuck retry, reap pass | rescind | It is the write-ahead guarantee |
| Fencing, quorum, replication, cluster heartbeats, out-of-band controllers, message buses, service posture | tenant | Ops, probes and `defprim`s in `.scind` files and hooks; never in the engine |
| Journal chain and signature | rescind | Tamper-evidence is a rescind guarantee; *sinks* are bindings |
...
When in doubt: the owner decides what rescind is *for* and how it *feels*; the implementer decides how it is *built*, within this document, and asks when the document is silent.
...
> rescind is the first language for operations against hosts in which "this can be undone" is a compile-time verdict rather than a comment — computed from declared footprints, undo loci and refusal modes rather than by search over a world model, and stating where the undo runs, past which step it cannot, what that step costs and who must acknowledge it, and how long the plan is bounded, as prose and as a stable structured form.

**One element of that sentence is narrower than it reads, and was narrowed on 2026-09-13.** A point past which undo cannot reach is not new: AT&T's `local protect` primitive (priority 2001; `docs/prior-art.md`) is exactly that, in a language. What is claimed here is **computing and reporting that point statically, before anything runs**, not the existence of such a point.
...
| Prior art | What it has | What rescind adds |
|---|---|---|
...
| Compensation calculi (Bruni, Melgratti & Montanari, POPL 2005; Sagas calculi; compensating CSP) | Semantics and expressiveness of compensations; decidability with static compensations | No footprints; no check that a given program's compensations compose; rescind is the checker the calculi lack |
...
| Database migrations (up/down) | Paired inverses | Unchecked; rescind verifies inverse composition and footprint disjointness |
...
| Miniscript / Antelope authorities | Threshold-and-timelock policy with load-time satisfiability | Policy, not operations; the model rescind borrows for gates |
| Metafont / Dhall | Total, deterministic languages | The totality ethic rescind adopts; neither is about effects |
...
**The method is part of the order, because twice the effort was fine and the method was not.**

1. **Search patent databases, not only the web and the literature.** The 2026-09-06 sweep recorded three infrastructure-workflow patents and missed a five-patent AT&T family whose title is "a programming language having fully undoable, timed reactive instructions" — nearer this claim than anything it did record. It was found on 2026-09-13, by accident, while searching a candidate project name.
2. **Run the plain-language search a stranger would run**, in addition to any structured query. A registry API answers *is this taken*; a plain search answers *what does this already mean*. The second question is the one that found both the name collision and the patent family, in one query each, after structured searches over an author-chosen list had missed both.
3. **Enumerate namespaces rather than lists of names you thought of.** A fixed list of six crate names reported three collisions where enumeration found fourteen.
4. **Say what was not checked.** No trademark search has ever been done here, and legal novelty has never been assessed by anyone on this project. "Novel" in this document means only "we looked and did not find", and that sentence is worth exactly as much as the looking behind it.

Done 2026-09-06 (`docs/prior-art.md`): two systems were found and the claim was narrowed. Amended 2026-09-13: the AT&T family was added and §1.1's point-of-no-return element narrowed.

The **name** sweep section 12 requires is in the same file. Done 2026-09-12 for `rue`, which was found to be taken by two other programming languages, one of them holding fourteen crates of its own toolchain; the project was renamed to **`rescind`** on 2026-09-13 and swept again, clean across every registry, with the residues recorded there.
...
| Term | Meaning |
|---|---|
| **rescind** | The language, and the client CLI |
| **rescind-core** | The pure library: model, checker, journal model, verdict rendering. No I/O, no clock |
| **engine** | The executor: runs plans through bindings, journals, arms backstops. Standalone daemon `rescindd`, or embedded via hooks |
...
| **tenant** | A project that consumes rescind. Its integration surface is `.scind` files plus hook registrations. No tenant name appears in framework code |
| **site** | The block of a `.scind` program that declares bindings and identities. Loaded before any plan is checked |
| **instance directory** | `<rescind_root>/instances/<id>/` on every run-capable host an instance touches: shim, staged files, snapshots, markers, region manifest, and the backstop artifact if any |
...
| **stdin preamble** | How secrets reach a remote `run`: NUL-separated pairs read from stdin by a rescind shim on the target before exec; never argv |
...
| **daemon dry-run** / **request dry-run** | `rescindd --dry-run`: no executors, no operators required. `rescind apply --dry-run`: a journal-only rehearsal against a real daemon that reserves nothing |

Reserved, not used in v0: **shrive** (a checked plan is *shriven*), **sexton** (a possible daemon name; `rescindd` is the working name).
...
| D-028 | The name is `rescind`; `.scind`, `rescindd`, `recant`, `knell`, `wane` | §2 | Chosen |
...
| D-032 | rescind is a reaper tenant from Phase 1 | §12 | The harness exists |
...
| D-036 | Rescind owns the gate *shape*; the binding owns proofs and identity | §5.11 | Policy mistakes surface at check time |
...
| D-053 | Whole-fact restore only when no other instance holds a region on the fact | §5.2 | Rescind's own artifact may not violate rescind's footprint rule |
...
| D-080 | Bootstrap is the tenant's provisioning; rescind verifies and prints | §7.7 | A verb claiming rights the design lacks is a lie |
...
| D-084 | Admin is the most powerful identity, on the record; `rescindd migrate` is exempt | §7.4, §7.12 | The loudest journal for the strongest identity |
...
```
SURFACE     .scind files: site block, defprobe/defprim/defop/defplan/defrole/defprotocol   DATA
    v
FRONT END   lexer, parser, lossless tree, resolver, clause dispatch, template expansion,  CODE (rescind-surface)
            diagnostics
    v
CORE        model (incl. Body and Prim), footprint algebra, refusal lattice, checker,     CODE (rescind-core, pure)
            verdict, journal model, state machine
    v
RENDER      artifact text per OS family, quoting, closure-check inputs                   CODE (rescind-render, pure)
    v
ENGINE      bindings, executors, control channel, hook protocol, journal sinks, arming,   CODE (rescind-engine)
            write-ahead lifecycle, reconciliation, dry-run, settle
    v
BINDINGS    generic built-ins only (file, local, ssh, rescind_toml, stdout, cron,             CODE (rescind-bindings)
...
rescind/
  ...
  core/          rescind-core     pure: model, checker, verdict, journal model, state machine
  render/        rescind-render   pure: artifact rendering per OS family, quoting; depends only on core
  surface/       rescind-surface  lexer, parser, tree, resolver, diagnostics
  hook-proto/    rescind-hook-proto  pure: the hook protocol's wire as data -- the ops of 7.5,
                              the registration frame, the resolved body, the reply records
  engine/        rescind-engine   lifecycle, bindings API, control channel, hook protocol, arming
  bindings/      rescind-bindings generic built-ins only
  cli/           rescind          the operator CLI (FreeBSD, Linux, Windows)
  daemon/        rescindd         the standalone engine daemon (FreeBSD, Linux, Windows)
  sdk/           hook-protocol clients over rescind-hook-proto: rust/ (reference), elixir/,
                 python/, java/, dotnet/, shim/ (rescind-hook)
  tenants/       acceptance tenants: .scind files + hook stubs + expected verdicts
  ...
  ci/            build-target.sh
  tree-sitter-rescind/            Phase 5
  .reaper.toml   bitbucket-pipelines.yml
```

Dependency direction is downward only. `rescind-core` depends on nothing in the workspace and on no I/O crate; `Body`, `Prim` and each primitive's argument classes are core types so closure and secret-placement analysis live in core. `rescind-render` depends only on core and performs no I/O. `rescind-surface` depends on core. `rescind-engine` depends on all three. `rescind-bindings` depends on `rescind-engine`. The CLI and daemon depend on everything. Tenants depend on the public API only.

### 4.3 Trust boundary

- `rescind-core` trusts nothing about the world. It reasons only over declared footprints, loci and modes. Its theorem is: *given honest declarations, the verdict is correct.*
- `rescind-engine` enforces honesty at runtime: after every `do` it diffs observed facts against the declared footprint; drift *outside* the footprint aborts the plan (the reversibility proof no longer holds); drift *inside* the footprint at undo time is another actor, handled by the step's drift policy (§5.2).
...
|---|---|---|---|---|
| `rescind` CLI | yes | yes | yes | cross-built; untested until a Mac exists (§11) |
| `rescindd` engine | yes (rc.d) | yes (systemd) | yes (Windows service) | cross-built; launchd daemon untested |
| Generic executors | `local()`, `ssh()` | `local()`, `ssh()` | `local()`, `ssh()` (OpenSSH for Windows) | `local()`, `ssh()` |
| Control channel | unix socket 0660 group `rescind`, peer-credential identity | same | named pipe, DACL for group `rescind`, client-SID identity | unix socket, `getpeereid` |
...
| Controller store | `/var/db/rescind` | `/var/db/rescind` | `%ProgramData%\rescind` | `/var/db/rescind` |
| Target `rescind_root` (bootstrapped once, §7.7) | `/var/db/rescind` root:`rescind`; `instances/` 2770; `lock` 0664 | same | `%ProgramData%\rescind` with group DACL | same as FreeBSD |
...
Per-OS knowledge lives in exactly three places: `rescind-render` (templates and quoting), the OS family's generic executor and scheduler bindings in `rescind-bindings`, and `ci/build-target.sh`; the one stated exception is `rescind-core`'s artifact vocabulary (which shell family an `os` implies, which languages have a template), there because the checker refuses at check time and core cannot depend on render. The OS family of a host comes from `HostRecord.os`; a `:target` backstop on a host whose declared language has no template for its OS is E0403.

---

## 5. Core model (rescind-core)
...
event ∈ { Checked, Requested, ProofAccepted{scope, authenticator, submitter}, Approved{rehearsal}, ApprovalExpired, Cancelled,
          ...
          StagedRemoved{step, reason}, InstanceDirOrphaned{host, instance, armed}, InstanceDirForeign{host, instance, controller}, Reclaimed{host, instance, forced, reason},
...
- `hash = H(prev_hash || canonical(entry without hash, sig))`; genesis `prev_hash = 0`. Canonicalisation is length-prefixed, field-ordered, domain-separated (`rescind-journal`).
...
- **Signing.** `journal … sign: key(path)` names an Ed25519 private key (OpenSSH format) held by the engine; `sig` is an SSHSIG over the canonical entry, namespace `rescind-journal`; `rescind journal verify` checks the chain and, given `--key`, every signature (`ssh-key` crate, in-process). Key rotation is a `KeyRotated` entry signed by the old key.
...
- **Rescind owns the shape; the binding owns trust.** The approval binding publishes the authenticator ids it can verify, each flagged `human: true|false`. `check` refuses a gate naming an unknown id, an unsatisfiable threshold, or one counting the requester (E0508). A gate satisfiable with no human refuses (E0509) unless the plan declares `allow_zero_human: true`; under `mode: :auto` it always refuses. The verdict reports satisfiability, minimum distinct humans, and the earliest instant each gate is satisfiable by wait alone.
- **Rescind supplies a request digest, not a challenge.** `request_digest = H(canonical(nonce, plan_id, instance, owner_host, params_hash, host_contract_hash, wane, requested_at, gate_hash, plan_content_hash))`, domain-separated `rescind-request`, with a 32-byte nonce from the engine's CSPRNG (core stays free of randomness). A step gate uses `step_digest = H(request_digest, step_index)`; an ack uses the same with scope `ack`. The binding renders whatever human-facing challenge it likes over the digest and verifies proofs against it; rescind never sees proof bytes, only `verified: true` for an authenticator id, and journals the submitting operator beside it.
- **Binding contract.** A proof MUST be bound to the digest and its scope (`plan`, `step`, or `ack`), so a proof for one request or step verifies for no other, and a plan, parameter or host-contract change after request invalidates every accumulated proof (the engine re-derives the host contract at request, approval and apply; a change is R0301). Replay resistance is a contract rescind requires and the conformance suite tests, not a mechanism rescind implements.
- **Plan entry.** Proofs accumulate across `rescind approve` calls; `wait` weight accrues from `requested_at`; the reap pass opens the plan the instant the threshold is crossed; the approval window (`gate …, window:`) lapses fail-closed; `wane` is anchored at approval, never at request. The requester's own authenticators never count toward a plan-entry or step `gate:`. The requester is therefore an input to `check` (`rescind check --as <authenticator>`, or the connected operator's identity), so E0508 is decidable offline.
- **Step gates.** `gate:` on a step enters `Waiting{gate}`; `wait` accrues from `StepGateRequested`; `rescind approve --step N` submits a proof in the `step` scope.
- **Acknowledgements.** A knell's `ack:` is a gate in the `ack` scope (default `thresh(1, humans())`, or `:none` with a reason for a knell acknowledged by its own guard, e.g. a fence driver's verified-off). Accepting a cost is the operator's own recorded decision, so the requester exclusion does not apply to acks. `rescind ack --step N --reason < token` is `rescind approve --step N` in the `ack` scope plus a journaled reason. Under `mode: :auto` every knell MUST declare `ack: :none` and every step gate MUST be satisfiable without a human (E0507). A knell with both `gate:` and `ack:` is legal: distinct scopes, distinct waits; the checker warns when both name the same single authenticator.
...
A `.scind` file is UTF-8, `\n` line endings, and begins with a version marker.

```
file      := "rescind" INT NEWLINE top*                     -- the language version this file targets (E0105 if missing or newer)
...
fpentry   := kind ":" factshape                           -- owned: file("/etc/x"), region: file("/etc/x", anchor: "rescind"), …
...
| E0607 | `inventory from: hook()` is checked with no record to check against; name one with `rescind check --inventory` |
...
```
rescind check   <plan.scind> [--host H] [--json]           parse, resolve, check; print verdict
rescind explain <plan.scind> [--host H] [--html]           per-host expanded steps with undo lines, loci, policies, gates; --html is the same as one self-contained page
rescind render  <plan.scind> [--host H]                    expected end-state facts; touches nothing
rescind artifact <plan> --host H --instance ID [--set k=v]... print the backstop artifact a :target backstop installs on H (Phase 1: over the plan IR)
rescind apply   <plan.scind> --host H [--set k=v]... [--mode auto] [--ack N:"reason"]... [--dry-run]
rescind reveal  <instance>                               fetch a secret held by hold(), exactly once
rescind approve <instance> [--step N] < token            submit a proof for a plan-entry or step gate
rescind ack     <instance> --step N --reason "…" < token approve in the ack scope plus a journaled reason
rescind recant  <instance> [--force=<name,…>]            run the undo; names are guards or the classes drift, unknown
rescind renew   <instance> --wane 2h                     extend a temporary plan; rearms the backstop first
rescind confirm <instance>                               disarm an unless_confirmed backstop
rescind commit  <instance> --reason "…"                  end a permanent plan from Held or Deferred
rescind resume  <instance>                               continue a Held instance from its held step
rescind handoff-done <instance> --step N                 continue a Deferred instance after the handoff
rescind abandon <instance> --reason "…"                  admin: close a Stuck or DriftHeld instance, world left as-is
rescind status  [instance]                               observed state, remaining wane, stuck and held sets
rescind journal verify <file> [--key K]                  verify the chain (and signatures) end to end
rescind reclaim <host> <instance> [--force --reason "…"] remove an orphaned instance directory
rescind bootstrap <host>                                 verify a target's rescind_root; print the admin commands if missing
rescind doctor  [--canary]                               bindings, executors, schedulers, bootstrap, sinks, modes
rescind fmt     <file>                                   lossless formatter
rescindd migrate [--dry-run]                             migrate the instance store schema, explicitly
```

Admin verbs (`abandon`, `reclaim`, `bootstrap`, `doctor --canary`, `journal verify` against the live store) require an operator declared `admin: true` (§7.4); `rescindd migrate` runs with the daemon stopped and is protected by OS permissions on the store.
...
pub trait Executor: Send {
    ...
    fn bootstrap_state(&mut self, host: &HostRecord) -> Result<BootstrapState, ExecError>;   // rescind_root, group, instances/, lock, modes
...
|---|---|---|
| `inventory` | Produces `HostRecord { name, address, os, roles: [Atom], reach: [Transport], facts: Map }` (Appendix C); names unique; `reach` non-empty | `rescind_toml(path)`, `hook(name)` |
| `journal` | Accepts a chained (and possibly signed) `Entry`; acks synchronously; refusal ⇒ engine refuses | `file(path)`, `stdout()`, `hook(name)` |
| `approval` | Publishes authenticator ids with `human` flags; renders a challenge over rescind's digest and scope; verifies one proof for one authenticator over that digest (`verified: true\|false`). Threshold evaluation is rescind's | `always()` (daemon dry-run only), `hook(name)` |
...
- **The stdin preamble.** `env:` and `stdin:` on `run` are implemented by the executor as NUL-separated `key=value` pairs read into the environment by a rescind shim on the target (in the instance directory) before exec; `ssh()` never uses `SendEnv`/`AcceptEnv` or command-line `VAR=val`.
...
**Bootstrap.** Rescind has no elevation mechanism. `<rescind_root>` (root-owned `0755`), the `rescind` group, `<rescind_root>/instances/` (`2770` root:`rescind`) and `<rescind_root>/lock` (`0664` root:`rescind`) are created once per run-capable target by the tenant's provisioning; `rescind bootstrap <host>` verifies them via `Executor::bootstrap_state` and prints the exact commands for that OS family when something is missing, never running them. "Target bootstrapped" is a host-viability precondition (built-in probe `rescind_root_ready`, R0407 if absent, reported by `rescind doctor`).

**Instance directory.** `<rescind_root>/instances/<instance-id>/` is created before the first step on every host with a run-capable executor (`local()`, `ssh()`, or a hook declaring `filesystem`), as the instance's `Owned` footprint there, mode `2770` group `rescind`, every file inside `0640` group `rescind`. It holds the stdin shim, `stage()`d files, `Modified` snapshots (size-capped, E0408/R0204), completion markers, a manifest of regions held on that host, the heartbeat file, the id of the controller that created it (§11: a host holding another controller's directory with an armed, unfired artifact refuses the apply, R0409), and — only if the plan has a `:target` backstop — the artifact. Files are updated by write-to-temp-then-rename in the same directory, never in place, so a root cron artifact and a group-member engine can each supersede or remove what the other wrote. On hosts with no run-capable executor, markers, snapshots and manifests live in the controller's store; a `:target` undo there is E0407 at check and R0408 at runtime, and the verdict says which per host. Wrong modes refuse arming (R0406). The directory is removed when the instance closes or commits.

**Locks.** `<rescind_root>/lock` is a host-wide lock (`flock(LOCK_EX)` on an `O_RDONLY` descriptor, so root and the ssh user share it; a named mutex with a group DACL on Windows), held by anyone reading or writing any instance's region manifest — and held across the *whole* of a region undo, from reading sibling manifests through the write. A per-instance lock in the instance directory protects that instance's markers and undo. Lock order is host, then instance, always. The tier-6 race stage exercises both.

**Backstop rendering.** A `:target` backstop is rendered by `rescind-render` from the undo bodies of every covered step, in reverse order, into one standalone artifact in the host's declared artifact language (POSIX `sh`; PowerShell; Python under `uv` with PEP 723 metadata, on any OS; §4.5), with every path and value baked in (quoted per family), a deadline file, a fired marker, the trigger logic, and per-step completion markers: the artifact undoes only steps whose marker is present and removes a marker after undoing its step. The instance-directory layout the artifact reads and the engine writes (`deadline` and `heartbeat` as epoch seconds in text, `markers/<n>` with one `<kind> <path> <sha256>` line per file fact, `snapshots/<n>/<k>`, `manifest`, and the region marker lines `# rescind-region <anchor> begin`/`end`) is the contract stated in `docs/DESIGN.md`. A non-file fact cannot be observed by a script and is undone as if intact. Installation is via the executor; arming writes the deadline; rearm rewrites only the deadline; disarm removes the artifact. The scheduler entry is the `backstop scheduler` binding's, and for PowerShell it invokes the script by `-EncodedCommand` or on stdin so execution policy never applies. The fired marker is read on the next engine contact and journaled `BackstopFired` (R0402).
...
**Reconciliation.** At boot the engine lists every instance directory on every reachable host and compares with the store. Each directory is stamped with the id of the controller that created it (`<store>/controller`, written once), and a directory stamped by another controller is not this store's business at all: it is left exactly as it is, journaled `InstanceDirForeign{host, instance, controller}` and listed by `rescind doctor`, never reclaimed and never undone -- whether its artifact is armed, fired or absent. An apply is refused on a host holding such a directory only while its artifact is armed and unfired (R0409): that is a live commitment, and a spent directory blocks nobody. A directory with no stamp predates the id and is read as this controller's. Of the remainder, a directory the store does not know about is *left in place* if it contains an armed, unfired artifact (journaled `InstanceDirOrphaned{armed: true}`, listed by `rescind doctor`); only directories with no artifact or a fired marker are reclaimed automatically. `rescind reclaim` is refused while an artifact is armed with its scheduler entry present (R0405); `rescind reclaim --force --reason` is accepted when the scheduler entry is absent or the operator states the artifact has been read, journaled `Reclaimed{forced: true}`. `abandon` disarms an instance's backstops where hosts are reachable and otherwise leaves them armed and says so (`Abandoned{artifacts_left_armed}`); a later firing is `BackstopFiredAfterAbandon`, accepted and visible.
...
| shim (`sdk/shim`, `rescind-hook`) | For `sh` tenants and anything else: registers as a hook and dispatches each request to a configured command | Static binary, all three OS families |
...
| A process in group `rescind` connects without a declared identity | Mitigated | Refused (R0503); group membership grants a connection, never an identity |
...
The store carries a schema version; `rescindd` refuses to start on an unknown or newer version (R0502) and never silently migrates an older one: `rescindd migrate` is explicit, dry-runnable, refuses if the store is not owned by the daemon account, and is journaled `Migrated{from, to, by}` on the daemon's next start. Every release ships the previous release's store as an upgrade vector.

### 7.14 Drills

A **drill** is an apply-and-recant of a real plan against a designated canary host, on whatever schedule an operator's timer keeps, leaving an attestation in the journal that the canary came back to what it was. It is what turns "the undo is written" into "the undo ran last night and restored the machine".

`rescind drill <file> --host <canary>` reads the digest of every fact the plan's footprint names on the hosts it touches, applies the plan, recants it, reads them again and compares. The drill is an ordinary instance from the engine's side -- it checks, requests, reserves, applies and recants like any other -- so it contends with production work in the ledger rather than stepping around it. Two refusals stand in front of it, both **R0410**: every host the plan touches must carry the role `canary` in the inventory, and the plan must be temporary, since a drill has to undo what it did and a permanent plan is not recanted. A step whose host is bound at runtime is refused the same way: a drill names where it runs before it runs.

The attestation is a journal entry and nothing else -- `DrillAttested{plan, host, instance, restored, facts}`, one line per fact of `<host> <shape> before=<digest> after=<digest>` -- so the chain covers it, and an attestation with a verification of its own would be a second chain to trust. `rescind journal verify --attestations` prints them once the chain verifies, and exits non-zero when a chain carries no drill or carries one that attested nothing. `restored` is true only when every fact was read and every digest came back: a fact the engine could not read (R0205) is kept as `unread=<why>`, attests to nothing, and makes the drill say so, as does a footprint that named no fact this engine could read. A drill that reads nothing has proved nothing about the undo it was there to prove.

What schedules it is the operator's own timer -- cron, a systemd timer, launchd -- invoking the verb. `rescindd` grows no scheduler of its own: rescind schedules *backstops* through the site's scheduler binding because they must fire with the engine dead, and a drill is the opposite case, needing the engine alive.

---
## 8. Acceptance tenants

Each tenant is a directory under `tenants/<name>/` with `.scind` files, hook stubs where embedded, an inventory, and `expected/` verdicts (JSON and prose) per host. Tenant names may appear **only** under `tenants/`, `docs/`, and test fixtures. They are the floor of capability the project is graded on, not its ceiling (§0.5).
...
**Deliverables.** `proto/` in Haskell (preferred: `Category`/`Arrow` instances exist) or OCaml; the four tenants as prototype terms *and* as unparsed `.scind` text beside them; `docs/verdict-schema.json` v1; `docs/prior-art.md` from the falsification day.
...
7. Write T1–T4 as `.scind` text against §6; every construct the terms need must have a spelling and every spelling must appear in §6. Grammar gaps found here are fixed in §6 before Phase 2.
...
### Phase 1 — rescind-core and rescind-render in Rust
...
7. `rescind-render`: artifact text per OS family and artifact language (`sh`, PowerShell, Python under `uv`) for the three triggers, completion markers, drift as §5.2, per-family quoting (E0109); the `sh` and Python artifacts executed in tests against a temporary instance directory.
...
**Goal.** `.scind` files check and explain through the front end; diagnostics are a golden-tested contract.

**Deliverables.** `surface/`; `rescind check/explain/render/fmt`; the four tenants as real `.scind` files; `docs/LANGUAGE.md`.
...
**Deliverables.** `engine/`, `bindings/` (generic only), `rescindd` on all three OS families, the full `rescind` CLI; `docs/hook-protocol.md` v1, `docs/control-protocol.md` v1; `docs/TESTING.md`; e2e harness under reaper; `sim/` (minimal).

**Tasks.**
1. Instance store: locked, atomic, versioned; boot recovery and settle (§7.8); `rescindd migrate` (§7.13).
...
4. Instance directories on run-capable hosts and controller-side markers for the rest; modes and setgid group; write-then-rename; host and instance locks with the region undo under the host lock; `Executor::bootstrap_state`, `rescind_root_ready`, `rescind bootstrap` as verify-and-print; `rescind doctor` mode and bootstrap checks; `stage()` lifecycle and recovery.
5. Backstop installation, arming, rearming and disarming through the `backstop scheduler` binding; generic `cron()` and `task_scheduler()` in `rescind-bindings`; heartbeat; skew probe; fired-marker journaling; reconciliation that never reclaims an armed artifact; `rescind reclaim` with its override; `abandon` disarm-first.
6. Journal: chain, optional signing, multi-sink synchronous delivery, `file()` and `stdout()` sinks, refusal on no ack; `rescind journal verify`.
...
8. Gates: approval binding contract with `human` flags; request, step and ack digests; accumulating proofs; windows; `always()` refused outside daemon dry-run; `rescind approve --step`, `rescind ack`, `--ack` up front.
...
**Acceptance.** *(Amended 2026-09-08, by the owner's decision of 2026-09-07: Windows stays under wine this phase.)* Everything in task 14 passes on the FreeBSD and Linux guests, and every R-code in Appendix D has a test that raises it. The Windows half is built for `x86_64-pc-windows-gnu`, unit-tested under wine, and proven no further this phase: `rescindd` as a service, the named-pipe channel with its access-control list and client-SID identity, the Task Scheduler backstop and the Windows Firewall variant of T3 wait for **Phase 3W**, a named later item that runs task 14 on a real Windows guest against no fakes. Nothing about the Windows design is left unwritten; what is missing is a machine to run it on.

**Exit criteria.** Acceptance met; `TESTING.md` published; tag v0.1.0.

**Not proven.** Schedulers other than cron and Task Scheduler. (`unless_heartbeat` under a real severed link and multi-controller contention were both Phase 5's, and are answered there: `tenants/e2e/tests/partition.rs` and R0409.) Task 4's *instance* lock is not implemented: the store's own lock admits one daemon, the artifact holds the host lock for its whole run and the engine holds it across any region undo, and two controllers on one host is Phase 5's question; the reasoning is in docs/DESIGN.md, and a lock nothing takes would be worse than none. On Windows, what wine cannot show: the service-control manager (the dispatcher and its stop handler are unit-tested by argument, and no manager has started rescindd), the access-control list as the kernel enforces it against a client that should be refused, the Task Scheduler, and PowerShell as `local()`'s shell. Wine *does* carry the named pipe end to end, list and all, and names the client from its own SID, so the identity model is proven on Windows and only its enforcement against a stranger is not. On macOS, `launchd()`, which is written and unit-tested and has installed no job.
...
**Goal.** A host process registers hooks and runs rescind plans as its own effects; T4 runs end to end with the tenant's stubs; T2 runs against a pseudo-cluster via hooks.

**Deliverables.** The `sdk/` set (§7.11), each passing the conformance suite; `rescind sdk-conform`; `tenants/t4/` with a minimal reactive host stub; `tenants/t2/` with hooks over a jail-based pseudo-cluster; hook protocol v1 frozen.

**Tasks.**
1. `rescind-hook-proto` and the Rust reference SDK.
...
3. Elixir, Python, Java and .NET SDKs and the `rescind-hook` shim, each with a conformance run in CI (the .NET run on the Windows guest); the shim built for all three OS families.
...
**Deliverables.** `tree-sitter-rescind`; an LSP with hover = footprint/undo/locus/drift and diagnostics (built on `lsp-server`, not `tower-lsp`, which has not been released since 2023; the comparison is in `docs/issues/0006`); `rescind explain --html`; drill mode (scheduled apply-and-recant against a canary with a chain-verified attestation); the `unless_heartbeat` partition stage; the complete `sim/`; upgrade vectors (previous release's `.scind` files and store must still check and migrate, or fail with a stated migration).

**Scheduled first (owner, 2026-09-11): package-native tests for the non-Rust SDKs.** Phase 4 shipped the Python, Elixir, Java and .NET SDKs with `rescind sdk-conform` as their only test (Phase 4, "not proven"). Before the tooling work, each gains a suite in its own idiom for what conformance cannot reach: the codec on inputs no conformance case sends (escapes and surrogate pairs, large and negative numbers, deep nesting, duplicate keys, a line that is not one JSON object, a truncated line); an unknown op or kind; the deadline; the secret rule (a secret cannot be formatted into a command line by accident); and the reply-shape rule each SDK enforces before it sends. Python: stdlib `unittest`. Elixir: ExUnit. Java: JUnit 6 (Jupiter; EPL-2.0, which reaches no consumer from test scope) in test scope only, so the published artifact keeps its zero dependencies -- and the Java SDK's hand-written JSON codec first, as the least-exercised code in the set. .NET: an xUnit project beside the package. Each suite runs in its SDK's pipeline step before conformance, and on the Ubuntu reaper guest beside `sdk/conform-all.sh` wherever the toolchain allows; where it does not (maven fetches plugins over the network the guest's conformance run avoids), the gap is stated, not skipped. Every defect a suite finds ships with its fix and a rediscovery row, and fixes to SDK behavior are releasable as v0.2.1 ahead of v0.3.0.

**First unit, as built (2026-09-11).** Python `unittest` (13 tests), ExUnit (13), JUnit 6 (23) and xUnit (13), run by `sh sdk/test-all.sh` on the Ubuntu reaper guest after conformance and in each SDK's pipeline step before it (`tests/tier3/t_test_all.sh` holds its selection). The guest gap the schedule anticipated did not arise: apt's maven fetches JUnit into a reaper cache, as the .NET SDK already fetched itself. What the suites found, each fixed with a test and a row: Java, Elixir and .NET did not carry 7.11's secret rule -- a run handler got the wire's `{text, secret}` maps, which print a secret's text -- and now hand it a `Resolved` that formats and serializes as `<secret>`, as Rust and Python did; the same three had no handler budget, and now have Rust's. A reply the SDK could not write ended the serve loop in Python, Java and Elixir, and in .NET, whose serialization sat outside the handler's try; a line nested deep enough ended it in Python (a `RecursionError`) and Java (a `StackOverflowError`); Python and Java wrote a NaN as a bare `NaN`, a line the engine cannot parse; Java's codec clamped an integral double beyond a long to `Long.MAX_VALUE` and reported a truncated `\u` escape as an index error; Elixir crashed on a handler answer of the wrong shape, and its `Client` on a request kind that was never an atom; and every .NET reply missing a required field threw instead of refusing, one id node being given two parents. The rediscovery runner gained the four SDK suites and `--suite`, and fixes for two defects of its own found while running them: a suite that read stdin (mix under Elixir 1.18) swallowed the rest of the row list while the battery reported success over the rows that had run, and a missing `patch(1)` was reported as a protection that had moved (`tests/tier3/t_rediscovery_run.sh`). Each package manifest now declares BSD-2-Clause. **Rows added** (verified by `sh tools/rediscovery/run.sh --tier 1 --suite ...` on the Ubuntu guest, and all but the .NET rows on the workstation too): `sdk-java-truncated-escape-is-an-index-error`, `sdk-java-deep-nesting-overflows-the-stack`, `sdk-java-nan-written-as-json`, `sdk-java-large-double-clamped`, `sdk-java-secret-formatted-by-accident`, `sdk-java-unwritable-reply-ends-the-hook`, `sdk-java-budget-ignored`, `sdk-python-deep-nesting-ends-the-hook`, `sdk-python-unwritable-reply-ends-the-hook`, `sdk-python-nan-written-as-json`, `sdk-elixir-malformed-answer-crashes`, `sdk-elixir-secret-formatted-by-accident`, `sdk-elixir-budget-ignored`, `sdk-elixir-unwritable-reply-ends-the-hook`, `sdk-elixir-unknown-kind-crashes-the-client`, `sdk-dotnet-refusal-reuses-the-id-node`, `sdk-dotnet-unwritable-reply-ends-the-hook`, `sdk-dotnet-secret-formatted-by-accident`, `sdk-dotnet-budget-ignored`. **Not proven:** the license metadata is read only where a build reads it -- the .NET pack and the maven package; no step builds the Python sdist or the Hex package. The .NET SDK still runs on Linux only (Phase 3W). The SDK fixes are unreleased; a v0.2.1 is the owner's call.

**Between the first and second units (owner, 2026-09-11): user docs for each SDK.** Every directory under `sdk/`, the shim's included, has a `docs/` of its own: a README with a quick start and the site lines and `rescindd run --spawn` that wire it in, a guide to every kind's handlers, refusals, the secret rule, the budget and the transports, and a page on testing a hook. The quick start is one audit hook (a journal sink and a notifier) written in each language, and it is a file each SDK's own suite drives; `tools/lint-sdk-docs.sh`, a gate phase, fails when a page's copy of it differs from the file by a byte, when an SDK has no docs, or when a link in them resolves to nothing. Writing them found the Rust SDK's serve loop and the `rescind-hook` shim ending at a line that is not JSON, where every other SDK skips it -- fixed, with a test and a row each (`sdk-rust-bad-line-ends-the-hook`, `sdk-shim-bad-line-ends-the-shim`) -- and the Elixir package declaring `elixir: "~> 1.15"` while its `JSON` needs 1.18, now corrected. **Not proven:** the Rust conformance hook's own read loop (`sdk/rust/src/bin/conform_hook.rs`) still ended at such a line -- closed since by `docs/issues/0012`. The Java and .NET SDKs have no socket transport, which the docs state rather than paper over.
...
**Second unit, as built (2026-09-11).** One reader, `Engine::fact_bytes`, serves every place the lifecycle reads a fact -- the snapshot, the R0201 witness before and after `do`, the markers, the undo's drift check -- and a read that fails is **R0205** there, never the fact's absence: before `do` the step is refused and nothing runs, after `do` the step has failed, at undo the undo fails and says so. The snapshot had silently kept nothing for an unreadable file, and a `:restore` then removed the file it should have put back. **`reads`** on a `defprobe` names the fact shape it reads (`reads guest.state(g)`); over `local()` and `ssh()` a fact that is no file and matches is read by running the probe with the shape's names bound (exit 0 present, 1 absent, anything else R0205), so drift on a jail's state is decided like any file's; hooks still answer by name. The plan IR is version 5 for it. Before `do`, every observed fact's digest is kept with the snapshot, and **a failed step whose facts all read as they did then is not undone** (`UndoSkipped`). **E0609** refuses at check a computed undo on a fact the host's executor cannot read. T2's text gains a `cbsd bstatus` probe for `guest.state(g)` and T3's a PowerShell one for `winfw.rule(rule)`; both tenants' verdicts are byte-identical, as E0608 left them -- the texts are completed, not changed in meaning. The T2 stage gains the obstacle-jail case: a start that fails on a jail someone else made leaves it, holds, and resumes once it is gone; the placement-refusal hold stays beside it. **Rows added:** `fact-read-failure-read-as-absent`, `failed-do-undone-though-it-never-took`, `nonfile-fact-read-by-no-probe`, `computed-undo-on-unread-fact-checks-clean`, `reads-line-ignored`. **Not proven:** a step whose `do` the engine died inside was still undone on the way back regardless, since what its facts read before `do` was in the memory that died -- closed since by `docs/issues/0011` (store schema 3). A failed step that observes no fact (an append-only footprint, as `record_succession`'s) is undone as before, because nothing shows it untouched. T2's `cbsd` probe and T3's Windows probe run nowhere: T2's own text is never executed, and Windows is Phase 3W's.

**Third unit, as built (2026-09-12): the two answers the guests had to give.** The in-repo tracker's `0009` and `0002`, taken together because both are decided by a real host rather than by an argument. *Multi-controller:* a store names its controller once (`<store>/controller`) and stamps it into every instance directory it creates; a host holding another controller's directory whose artifact is armed and unfired refuses the apply (R0409), naming the directory and the `rescind reclaim --force --reason` that takes it, and boot reconciliation leaves every foreign directory exactly as it is (`InstanceDirForeign`, reported by `rescind doctor` apart from the orphans), spent ones included. The first draft refused on any foreign directory and the e2e guests refuted it inside one cycle -- every stage there is a store of its own, and a fired directory a killed controller had left would have closed the guest to every later stage for good; what is dangerous is a live commitment, not a spent one, and two controllers acting at once with nothing armed between them stays undecided, which is the lock protocol §11 defers. *The partition stage:* `tenants/e2e/tests/partition.rs` applies a plan with `[after: 1h, unless_heartbeat: 60s]`, cuts the controller's path to the target with a firewall rule naming the target address and port 22 alone, and requires the heartbeat file to stop advancing, the target's own cron to fire the artifact on the stale beat and undo the step with no engine involved, and the engine to read the firing as `backstop_fired` (R0402) once the link is back. It runs on both guests; the cut is a filter on the loopback path both ends share, not a vnet, so what is proven is that the engine is alive and unreachable and the target acts alone. Its first draft put the severing rule in a chain inside `/etc/nftables.conf` -- the very fact T3's plan rewrites and reloads -- and the Linux guest lost it mid-run, which is the rule the harness now keeps: a stage's mechanism may not live inside the fact another stage mutates. **Rows added:** `second-controller-not-refused`, `foreign-instance-dir-reclaimed`, `spent-foreign-dir-blocks-the-host`. **Not proven:** two controllers on one host with nothing armed between them; the dead man across a real routed partition, as opposed to a filtered loopback path.

**Fourth unit, as built (2026-09-12): drills.** `docs/issues/0003`, designed on the issue and built from it. §7.14 holds the rule; `rescind drill <file> --host <canary>` holds the verb; `DrillAttested` holds the evidence, in the chain and nowhere else, printed by `rescind journal verify --attestations` once the chain verifies. A drill is an ordinary instance -- checked, requested, reserved, applied, recanted -- with two refusals in front of it (R0410: a host without the role `canary`, a host bound at runtime, a permanent plan) and one honesty rule behind it: a fact the engine could not read attests to nothing, and neither does a footprint with no readable fact in it. It is a verb and not a keyword, so the grammar stays settled under the editor tooling still to come, and it is scheduled by the operator's own timer, because `rescindd` schedules backstops -- which must fire with the engine dead -- and a drill needs the engine alive. `tenants/e2e/tests/drill.rs` drills a real canary over a real sshd on both guests and reads the attestation back out of the verified chain. **Rows added:** `drill-on-a-non-canary`, `drill-attests-an-unread-fact`. **Not proven:** a drill of a plan whose steps touch more than one canary, and a drill under a gate -- both admitted by the code and exercised by neither suite.

**Seventh unit, as built (2026-09-12): the language server, the name sweep, and what the pipeline found.** `docs/issues/0006`, `0008`, `0007`. `rescind explain --html` renders the listing as one self-contained page that fetches nothing. `lsp/` (`rescind-lsp`) serves the front end's own diagnostics and `explain`'s own words over `lsp-server` -- not the `tower-lsp` this section named, which has had no release since August 2023; the comparison and the darwin dependency check the issue demanded are on it. Two decisions came out of writing its tests: a clause-dispatched file cannot resolve without a host, which is an argument `rescind check` is given and an editor is not, so the server names one itself and says so rather than underlining every such tenant; and an unsaved buffer is parsed and not checked, because a resolve reads imports from disk. The Ubuntu guest then found a test helper pasting a path after `file://` where an editor percent-encodes, and the library gained `uri_of` beside `path_of` with a round trip over the characters that need it. The **name sweep** section 12 requires is recorded in `docs/prior-art.md`, and at this point the project was still called **`rue`**: that name was found taken on crates.io, PyPI and npm and by two other programming languages on GitHub, one of which held `rue` and `rue-lsp`. Recorded, not decided -- and the public README was filed as `docs/issues/0016` rather than written against a name that might change. It did change: the project was renamed to `rescind` at v0.4.0, and the sweep that produced that decision, along with the two defects in its own method, is in `docs/prior-art.md`. **What the pipeline found:** `tree-sitter-rescind` compiles a generated C parser, and the darwin targets are cross-linked with zig and no macOS SDK, where `cc-rs` reaches for a `cc` that takes `-arch` and this host's does not. It is excluded from the cross-target builds and from nothing else -- no binary ships it -- and `tools/lint-cross-build.sh` (gate phase `cross-build`) now fails that case in the gate instead of twenty minutes into the pipeline. **Rows added:** `explain-html-renders-a-plans-markup`, `lsp-underlines-a-missing-host`. **Not proven:** any editor actually driving the server or painting the grammar; the tests call the handlers and load the queries.

**Sixth unit, as built (2026-09-12): the grammar for editors.** `docs/issues/0005`. `tree-sitter-rescind/` holds `grammar.js`, the generated parser checked in beside it, `queries/highlights.scm` and a README with the Neovim and Helix stanzas. It parses every tenant text and every corpus snippet the front end accepts, and no text the front end refuses with a parse error. The guard that holds it (`tree-sitter-rescind/tests/drift.rs`) runs both parsers over every `.scind` text in the repository and requires them to agree, because two parsers over one language drift apart the moment nobody compares them. E0105 is outside that comparison and says so: the grammar requires the version marker and does not read its number, so no editor goes blind the day the version turns over. The grammar stays out of the checker's business by design -- it decides what an editor shows, and `rescind check` decides what a text means. **Not proven:** highlighting as an editor actually paints it; the queries load against the grammar, which is what a test can say. Regenerating the parser needs Node and the tree-sitter CLI, which the gate does not require and does not have on the workstation (npm there is broken; `cargo install tree-sitter-cli` is the way in).

**Fifth unit, as built (2026-09-12): the simulation's world, and its plants.** `docs/issues/0004`. Tier 7 checked twenty invariants against a world of two plans with no repeat, no deferred step, no fact that was not a file and no host without a filesystem; four invariants it could not reach at all. A third plan, cut from T2's and T4's shapes, carries all of those, and `DropRead` puts unit 2's R0205 rule under an arbitrary ordering. Every invariant the world reaches -- eighteen of twenty -- now has a violation planted by hand, in the world, the record, the ledger or the journal and never in the engine, and the check for it must catch it; 6 and 18 remain named with where they are proven instead. The widening found four defects, all in the shadow: a footprint shape naming a repeat's variable read literally; a step performed out of band on an unreachable host asserted as a fact; invariants 7 and 10 reading one host's ordering as one instance's; and invariant 12's plant skipping itself in silence for want of two live region holders. **Rows added:** `sim-reads-a-repeat-shape-literally`, `sim-judges-an-artifact-across-instances`, `sim-skips-a-plant-in-silence`. **Not proven:** the rendered artifact is still modelled rather than executed in the sim (`render/tests/execute.rs` and the e2e harness execute one), and 39 seeds of 24 events is a sample, not a proof.
...
| Sidecar vs spawned-child default for `rescindd` hooks in docs | 4 | Both via the same protocol |
...
| Multi-controller: refuse, or a lock protocol over a shared fact? | 5 | **Settled 2026-09-11:** refuse what a target can decide. Each store names its controller and stamps it into every instance directory it creates; a host holding another controller's directory with an armed, unfired artifact -- a backstop that will undo work there on its own schedule -- refuses the apply (R0409), and boot reconciliation leaves every foreign directory alone (`InstanceDirForeign`), spent ones included, since they are not this store's to reclaim. Two controllers acting at once with nothing armed between them is what a lock protocol over a shared fact would decide, and that is still undesigned, to be taken when a tenant needs it. Closed as `docs/issues/0009` |
| Editor tooling before or after v0.1.0? | 5 | After; keep the grammar settled first |
| Python artifacts: `uv` and a cached interpreter as a bootstrap precondition on every OS; `uv run --offline` at fire time on a locked-out host; pre-warming at arm; what `rescind bootstrap` prints when they are missing | 3 | The artifact is standard-library-only and the tests run it under `uv run --offline --script`; the target-side preconditions are the scheduler binding's and the bootstrap probe's to check |
...
- **Name sweep**: crates.io, PyPI, npm, GitHub for `rescind`, `rescindd`, `rescind-core`; record in `docs/prior-art.md` before anything public.
- **Repository**: primary on Bitbucket (`axonibyte/rescind`), mirrored to GitHub by the `doMirror` pipeline step on every push (`git clone --mirror`, `git push --mirror`; repositories and deploy keys assumed provisioned). GitHub description: `[ Mirror ] A language for provably reversible operations`. License BSD-2-Clause, copyright Axonibyte Innovations, LLC.
...
- **Issues** *(owner, 2026-09-11)*: in the repository, `docs/issues/`, a file per issue and an index the gate's `issues` phase holds to the files -- Bitbucket Cloud retired its issue tracker in August 2026. This file stays the plan of record; an issue is a unit of work or a decision, and links here rather than restating it. The rest of Phase 5 is filed there as #0001-#0009, with the open decisions and known gaps beside it.
- **Versioning**: SemVer for the crates; verdict schema, hook protocol and control protocol versioned independently; `.scind` and store upgrade vectors from v0.1.0 onward.
...
Backstops and targets: `R0401` backstop scheduler absent · `R0402` backstop fired (informational) · `R0403` target clock skew beyond tolerance at arm · `R0404` backstop rearm failed (renewal refused) · `R0405` reclaim refused (armed artifact, scheduler present) · `R0406` instance directory modes wrong (arming refused) · `R0407` target not bootstrapped · `R0408` `:target` undo on a host whose executor reports no filesystem, detected before `do` (step refused, prefix reverts) · `R0409` the host holds an instance directory stamped by another controller with an armed, unfired artifact (apply refused; rescind is single-controller per host) · `R0410` the plan cannot be drilled: a host it touches is not declared `canary`, its host is bound at runtime, or it is permanent and so not recantable (§7.14).

Protocol and store: `R0501` control protocol version mismatch · `R0502` store schema version unknown (run `rescindd migrate`) · `R0503` control client identity does not match a declared operator · `R0504` act outside the client's `operator_for` scope · `R0505` hook registration from an undeclared registrar or outside its `may_register` · `R0506` admin verb attempted by a non-admin identity.
