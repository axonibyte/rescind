...
cross-built with no SDK. A new workspace crate (`lsp/`, `rescind-lsp`).
...
and its users have moved to a fork or to `async-lsp`. rescind's workspace has no
async runtime at all -- `grep tokio Cargo.lock` finds nothing -- and a
language server is the worst possible reason to acquire one: judging a rescind
text takes a millisecond, and an editor that asks twice gets two answers in
order from a plain loop. And the dependency: `rescind-lsp` pulls exactly
`lsp-server`, `lsp-types`, `serde` and `serde_json` beside rescind's own crates,
...
**Closed.** Built, 2026-09-12, on `lsp-server` + `lsp-types` rather than the `tower-lsp` the roadmap named; the comparison is in the table above and the short of it is that tower-lsp has had no release since August 2023, the workspace has no async runtime, and judging a rescind text takes a millisecond. `rescind-lsp` pulls four crates beside rescind's own, and `tools/lint-darwin-deps.sh` stays clean.
