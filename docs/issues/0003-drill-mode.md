...
of its own), what schedules it (`rescindd` or an outside timer), what the
attestation holds (each observed fact's digest before apply and after
recant, bound to the chain) and how `rescind journal verify` checks it. A
...
*A verb, not grammar.* `rescind drill <file> --host <canary> [--plan P]`. No new
...
launchd -- invoking `rescind drill`, with the line to use in the docs. `rescindd`
grows no scheduler of its own: rescind schedules *backstops* through the site's
...
entry and nothing else, so the chain covers it and `rescind journal verify`
already proves it was not edited after the fact -- an attestation with a
verification of its own would be a second chain to trust. `rescind journal
