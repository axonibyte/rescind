# 0005: tree-sitter-rescind: a grammar for editors
...
**Where.** `tree-sitter-rescind/` at the repository root: `grammar.js`, the
...
**Closed.** Done, 2026-09-12. `tree-sitter-rescind/` holds `grammar.js`, the parser generated from it and checked in, `queries/highlights.scm`, a `package.json` for npm, a Cargo crate so the guard can load it, and a README with the Neovim and Helix stanzas. It parses every tenant text and every corpus snippet the Rust front end accepts, with no error node, and refuses every text the front end refuses with a parse error.

The guard (`tree-sitter-rescind/tests/drift.rs`, an ordinary workspace test needing only a C compiler) runs both parsers over every `.scind` text under `tenants/` and `surface/tests/corpus/` and requires them to agree on which files are rescind; it also loads the highlight queries against the grammar, so a query naming a node the grammar lacks is caught here rather than by every editor in turn, and checks that the checked-in parser and `grammar.js` describe the same rules -- which is what the gate can say without Node.

Three decisions worth recording. E0105 is outside the comparison and named in the test: the grammar requires the version marker and does not read its number, because a grammar that refused `rescind 7` would blind every editor the day the version turns over. `repeat over: ... as g` with no `max:` parses here, because the front end parses it and the checker refuses it (E0106) -- a grammar stricter than the parser would have an editor underline what rescind reports as a diagnostic. And the contextual-keyword rule of 6.2 shapes the lexer: `owned:`, `os:` and every other name-plus-colon is one token, so the footprint kinds and the pattern keys are written that way.
