# 0014: Phase 3W: task 14 on a real Windows guest
...
including the `winfw_rule` probe the second unit added, which has never
run -- PowerShell as `local()`'s shell, and the .NET SDK on Windows. Needs
a Windows machine.
