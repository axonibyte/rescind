...
//! `.scind` snippet under `tests/corpus/` with its tree dump (`.tree`) and
//! its diagnostics (`.diag`) beside it. The goldens are read-only in the
//! suite; `RESCIND_UPDATE_GOLDENS=1` rewrites them, as it does the tenants'.
...
use rescind_surface::{dump, format, parse};
...
fn hold(path: &Path, actual: &str) -> Result<(), String> {
    if std::env::var("RESCIND_UPDATE_GOLDENS").as_deref() == Ok("1") {
    ...
    match fs::read_to_string(path) {
        ...
        Err(_) => Err(format!(
            "{} is missing; run with RESCIND_UPDATE_GOLDENS=1 to write it",
            path.display()
        )),
    }
}
...
fn every_snippet_matches_its_tree_and_diagnostics_goldens() {
    let mut names: Vec<PathBuf> = fs::read_dir(corpus_dir())
        .unwrap()
        .map(|e| e.unwrap().path())
        .filter(|p| p.extension().is_some_and(|e| e == "scind"))
        .collect();
    names.sort();
    assert!(names.len() >= 12, "the corpus has at least twelve snippets");
    let mut failures = Vec::new();
    for rescind in &names {
        let src = fs::read_to_string(rescind).unwrap();
        let name = rescind.file_name().unwrap().to_str().unwrap();
        ...
        let diag: String = p
            ...
            .map(|d| format!("{}\n", d.render()))
            .collect();
        if let Err(e) = hold(&rescind.with_extension("tree"), &tree) {
            failures.push(e);
        }
        if let Err(e) = hold(&rescind.with_extension("diag"), &diag) {
            failures.push(e);
        }
