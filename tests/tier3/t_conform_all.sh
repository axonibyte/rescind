...
# against stub interpreters and a stub `rescind` that records what it was
...
tmp=$(mktemp -d "${TMPDIR:-/tmp}/rescind-t-conform.XXXXXX") || exit 2
...
printf '#!%s\nprintf "%%s\\n" "$*" >> "%s"\nexit 0\n' "$shell" "$tmp/judged" > "$tmp/rescind" &&
    chmod +x "$tmp/rescind" || exit 2
printf '#!%s\nexit 1\n' "$shell" > "$tmp/rescind-fails" && chmod +x "$tmp/rescind-fails" || exit 2

# conform(<path> <rescind> <args...>): run the copy with PATH set to exactly
...
# 3. Two named.
conform "$full" "$tmp/rescind" python dotnet
...
conform "$tmp/tools" "$tmp/rescind" python
...
conform "$full" "$tmp/rescind-fails" elixir
...
conform "$full" "$tmp/rescind" pyhton
