...
rescind's tree-sitter grammar: highlighting and navigation for editors
...
*means* — `rescind check` does, and it is the only authority on that. A file
this grammar parses without complaint can still be refused by the checker,
and should be: the diagnostics are the point of rescind, and an editor that
...
the repository root under `tree-sitter-rescind/`, with `queries/highlights.scm`
beside it.

Neovim, with `nvim-treesitter`:

```lua
require('nvim-treesitter.parsers').get_parser_configs().scind = {
  install_info = {
    url = 'https://bitbucket.org/axonibyte/rescind',
    location = 'tree-sitter-rescind',
    files = { 'src/parser.c' },
    branch = 'main',
  },
  filetype = 'rescind',
...
[[grammar]]
name = "rescind"
source = { git = "https://bitbucket.org/axonibyte/rescind", subpath = "tree-sitter-rescind", rev = "main" }
```
...
cargo test -p tree-sitter-rescind
```
...
`tests/drift.rs` parses every `.scind` text under `tenants/` and
`surface/tests/corpus/` with both this grammar and the Rust front end, and
requires them to agree on which files are rescind. A text the front end accepts
...
`rescind 7` would blind every editor the day rescind's version turns over.
