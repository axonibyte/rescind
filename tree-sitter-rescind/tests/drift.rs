...
//!
//! rescind has two parsers now: the Rust front end, which decides what a text
//! means, and this grammar, which decides what an editor shows. They read
//! the same files, so they must agree about which files are rescind -- and the
...
//! What it catches is the thing that matters to an operator: a text rescind
//! accepts and the editor underlines, or a text rescind refuses and the editor
//! shows as clean.
//!
...
//! compiler reads the text -- and a grammar that refused `rescind 7` would
//! leave every editor blind on the day rescind's version turns over, while one
//! that accepted a file with no marker at all would call something rescind that
...
/// Every `.scind` text under the repository, tenants and corpus alike.
fn every_rescind_text(root: &Path) -> Vec<PathBuf> {
    let mut out = Vec::new();
...
fn walk(dir: &Path, out: &mut Vec<PathBuf>) {
    ...
    for e in entries.flatten() {
        ...
        if p.is_dir() {
            walk(&p, out);
        } else if p.extension().is_some_and(|x| x == "scind") {
            out.push(p);
        }
    }
}
...
fn parser() -> tree_sitter::Parser {
    ...
    p.set_language(&tree_sitter_rescind::LANGUAGE.into())
...
fn the_grammar_accepts_every_text_the_front_end_accepts() {
    ...
    let mut p = parser();
    let mut checked = 0;
    for path in every_rescind_text(&root) {
        let src = std::fs::read_to_string(&path).expect("the text");
        let rescind = rescind_surface::parse(&src, &path.to_string_lossy());
        let tree = p.parse(&src, None).expect("a tree");
        let ours = !has_error(tree.root_node());
        let rel = path.strip_prefix(&root).unwrap_or(&path).display();
        let version_only = !rescind.diagnostics.is_empty()
            && rescind
                .diagnostics
                .iter()
                .all(|d| d.code == rescind_core::diagnostics::Code::E0105);
        if version_only {
            continue;
        }
        assert_eq!(
...
fn the_highlight_queries_load() {
    let language: tree_sitter::Language = tree_sitter_rescind::LANGUAGE.into();
    tree_sitter::Query::new(&language, tree_sitter_rescind::HIGHLIGHTS_QUERY)
