...
point it cannot -- is decided by `rescind check` before anything runs, and stated
in one sentence.

The honesty caveat, on the first screen where it belongs: the checker's
theorem is *given honest declarations, the verdict is correct*. Rescind reasons
...
every `.scind` file in the repository. A language server answers an editor
...
clauses dispatch on one. `rescind explain --html` renders a plan as a page that
fetches nothing, for a change record or an incident's notes.

`rescind drill` applies a plan to a host the inventory declares a canary,
recants it, and leaves an attestation in the journal naming every fact it
touched with the digest before and after; `rescind journal verify
...
Two other programming languages are already called Rescind, and `rescind` is taken
on crates.io, PyPI and npm; the sweep is in
[`docs/prior-art.md`](docs/prior-art.md), with what is free and what a
public rescind would walk into. The name has not been decided, and nothing here
depends on the decision.

## Status: Phase 4

A host process can embed rescind. It holds one connection to `rescindd` and is, on
...
case: a Rust SDK, the `rescind-hook` shim for shell tenants, and SDKs for
Python, Elixir, Java and .NET, each judged by `rescind sdk-conform` against
...
daemon that a person gets from `rescind check` at a terminal.
...
Rescind applies and reverts plans against real hosts. `rescindd` is a daemon over
a locked instance store with a chained, optionally signed journal; `rescind`
...
restores the file once, not twice; and `rescind doctor --canary` proves a
...
The Rust workspace checks plans: `rescind-core` is the checker, `rescind` the
command line (`check`, `explain`, `states`) over a plan IR, `rescind-tenants`
...
from the unparsed `.scind` text beside their goldens under `tenants/`, and
...
(`rescind-render`, `rescind artifact`, the `sh` and Python artifacts executed in
...
complete as far as a workstation and a pipeline can prove it: `rescind-surface`
parses every `.scind` text under `tenants/` into a lossless tree, `rescind fmt`
is the identity on each, the resolver turns a text into the checker's
input for one host (`rescind check file.scind --host H`), the texts are the
...
elevation engine. Those are ops, probes and bindings a tenant declares. Rescind
sequences operations and proves their reversal. Where convergence is the
right model, rescind is the wrong tool.

## Prior art, and the delta from each

The claim is narrow on purpose: rescind is the first language for operations
...
| The `.scind` text | The source: every golden is derived from a text by the front end (`rescind check tenants/t1/plan.scind`); the Rust terms that carried Phase 0's record retired once the front end reproduced each of them. What the grammar admits beyond the constructs the tenants use is proven by the parser corpus and the diagnostic goldens only |
...
| The other 5 codes | Not modeled, each with its reason in `rescind_tenants::UNMODELED_CODES`: E0402 and E0408 are engine time, E0406 cannot arise (installation precedes the first covered step by construction), E0409 is fixed by `--host`, E0411 needs sinks the site does not declare |
...
| A drill of a many-host or gated plan | `rescind drill` admits both and neither suite exercises one |
...
RESCIND_FUZZ_STEPS=5000 cargo test --workspace --locked --test fuzz   # the seeded properties, longer
...
cargo run -q -- fmt tenants/t1/plan.scind                       # the canonical layout of a .scind file; --check to only compare
cargo run -q -- check tenants/t3/plan.scind --host fw-01 --json # a .scind text resolved for one host; --plan-name when the file defines several
cargo run -q -- explain tenants/t3/plan.scind --host fw-01 --html > plan.html  # the same listing as a page that fetches nothing
...
`RESCIND_UPDATE_GOLDENS=1 cargo run -p rescind-tenants --bin rescind-goldens` is the only
...
in `RESCIND_CHECK_SKIP_OK`; the FreeBSD reaper guest declares its skips that way
...
| `tools/lint-ecodes.sh` | The E-code guard: `Rescind.Proto.Diagnostics` and the roadmap's table must agree |
...
| `sdk/` | The embedding SDKs of 7.11 -- `rust/`, `python/`, `elixir/`, `java/`, `dotnet/` -- and `shim/`, the `rescind-hook` shim; each has user docs in its own `docs/` |
...
| `Cargo.toml`, `core/` | The Rust workspace and `rescind-core` (Phase 1): the model and its plan-IR shape, the checker, the verdict and its prose, `explain`, the state machine, the ledger; pure, no I/O |
| `cli/` | `rescind`, the operator CLI: the whole verb list of section 6.8, over a `.scind` file for the offline verbs and over the control channel for the rest; exit codes per that section |
| `surface/` | `rescind-surface` (Phase 2): the lexer, the lossless tree, the parser, `rescind fmt`, and the resolver that turns a `.scind` text into the checker's input for one host |
| `render/` | `rescind-render` (Phase 1): the target-side backstop artifact in POSIX `sh`, PowerShell and Python, every value baked in and quoted for its family |
| `engine/` | `rescind-engine` (Phase 3): the clock, the store, the journal and its sinks, the executor seam, the lifecycle, footprints and drift, backstops and schedulers, gates and proofs, secrets, the control channel and the hook protocol |
| `bindings/` | `rescind-bindings`: the generic built-ins and no more -- `local()`, `ssh()`, `cron()`, `task_scheduler()`, `launchd()`, `file()`/`stdout()` journals, `key()`, `always()`, `requester()`, `hold()`, `stdout()` notify |
| `daemon/` | `rescindd`: the daemon over a site block, its rc.d, systemd and `sc.exe` installation files |
| `tree-sitter-rescind/` | The grammar for editors: `grammar.js`, the parser generated from it, highlight queries, and a guard that holds it to the front end over every `.scind` text here |
| `lsp/` | `rescind-lsp`: the language server -- the checker's diagnostics as a text is edited, and hover on a step |
| `sim/` | `rescind-sim` (tier 7): seeded event lists against a real engine, the twenty invariants of 10.3 after every event, and a shrinker over the events that broke one |
...
| `tenants/harness/` | `rescind-tenants`: the tenants and negatives as Rust terms, the case table as code, the tests that hold `rescind-core` to every golden byte for byte, and `rescind-goldens`, the only writer |
...
| `tenants/` | The acceptance tenants' `.scind` text, inventories and expected verdicts (Phase 0) |
