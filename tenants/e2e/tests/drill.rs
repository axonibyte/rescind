...
//! attestation is read back out of the chain by `rescind journal verify
//! --attestations`, which is what the acceptance line asks for: journaled,
//! and verified.

use std::fs;

use rescind_e2e::{bin, must, require_provisioned_host, rescind, Daemon, Site};
...
fn canary_host(os: &str, rescind_root: &str) -> String {
    format!("{CANARY}os = \"{os}\"\nrescind_root = \"{rescind_root}\"\n")
}
...
fn site(name: &str, f: &str) -> Site {
    Site::with(
        ...
        "",
        &canary_host(
            rescind_e2e::os_family(),
            &rescind_e2e::rescind_root().to_string_lossy(),
        ),
        "",
    )
}
...
fn a_drill_applies_to_the_canary_recants_and_attests_in_the_chain() {
    ...
    assert!(
        text.contains("file:/etc/rescind-e2e-drill before=") && text.contains(" after="),
    ...
    // reading it back is `rescind journal verify --attestations`.
    let journal = site.dir.join("journal.ndjson");
    let out = std::process::Command::new(bin("rescind"))
        .arg("journal")
        .arg("verify")
        .arg(&journal)
        .arg("--attestations")
...
fn a_drill_is_refused_on_a_host_the_inventory_does_not_call_a_canary() {
    require_provisioned_host();
    let f = "/etc/rescind-e2e-drill-refused";
    let _ = fs::remove_file(f);
