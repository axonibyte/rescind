#!/bin/sh
# rescind backstop artifact: plan breakglass on db-01 (os freebsd), instance golden, language sh. Rendered by rescind-render; do not edit.
set -u
ROOT='/var/db/rescind'
...
# step 2: authorized_keys_block
M="$INST/markers/2"
if [ -f "$M" ]; then
  skip=0
  if [ "$skip" -eq 1 ]; then defer 2; else
    strip_region '/root/.ssh/authorized_keys' 'rescind-breakglass' || { if [ -n "${RESCIND_NOLOCK:-}" ] || foreign_region '/root/.ssh/authorized_keys'; then defer 2; else restore '/var/db/rescind/instances/golden/snapshots/2/0' '/root/.ssh/authorized_keys'; clobbered 2; fi; }
    rm -f "$M"
  fi
fi
# step 1: service_posture
M="$INST/markers/1"
if [ -f "$M" ]; then
  skip=0
  [ "$(sha '/etc/ssh/sshd_config.d/rescind-breakglass.conf')" = "$(recorded "$M" '/etc/ssh/sshd_config.d/rescind-breakglass.conf')" ] || clobbered 1
  if [ "$skip" -eq 1 ]; then defer 1; else
    rm -f '/etc/ssh/sshd_config.d/rescind-breakglass.conf'
    rm -f "$M"
  fi
fi
: > "$INST/fired"
