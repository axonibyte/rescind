#!/bin/sh
# ci/sdk-conform.sh  judge one SDK against the rescind this pipeline built.
...
# build rescind: it takes the Linux x86_64 binary buildLinuxAmd64 left in dist/,
...
set -- "$root"/dist/rescind-v*-x86_64-unknown-linux-gnu
if [ "$#" -ne 1 ] || [ ! -f "$1" ]; then
    echo "sdk-conform: expected one dist/rescind-v*-x86_64-unknown-linux-gnu, found: $*" >&2
    exit 1
fi
rescind=$1
chmod +x "$rescind"
...
exec sh "$root/sdk/conform-all.sh" --rescind "$rescind" "$sdk"
