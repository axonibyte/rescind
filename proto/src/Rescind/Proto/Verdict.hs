...
-- canonical JSON. The prose is Rescind.Proto.Prose; @explain@ is
-- Rescind.Proto.Explain. Every "the verdict says" in the roadmap names a field
-- here; a new clause without a field is a schema bump.
module Rescind.Proto.Verdict
  ( Verdict (..)
  , Status (..)
  , Diagnostic (..)
  , PointOfNoReturn (..)
...
import Rescind.Proto.Diagnostics (Code, codeText)
import Rescind.Proto.Intent (Intent (..))
import Rescind.Proto.Model (Duration (..))
