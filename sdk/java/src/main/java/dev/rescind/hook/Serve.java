...
/**
 * The way {@code rescindd} reaches a hook it spawned (docs/hook-protocol.md,
 * "A hook over stdio").
 *
...
public final class Serve {
    ...
    /**
     * Serve as a child the daemon spawned ({@code rescindd run --spawn}).
     *
     ...
     * engine's deadline ({@code rescindd run --hook-deadline}) is not on the
    ...
    public static void stdio(String name, Hooks hooks, Duration budget) throws Exception {
        ...
        if (ack == null || !acknowledged(ack)) {
            System.err.println("rescind-hook: registration was not acknowledged: " + ack);
            System.exit(1);
        }
