...
records of `rescind-hook-proto`, and a serve loop that does the registration
...
- **Typed.** Requests and replies are the protocol's own records
  (`rescind_hook_sdk::proto`), not `serde_json::Value`, except where the
...
The crate is `rescind-hook-sdk`, part of rescind's workspace, and is not on
crates.io. Depend on it from a checkout of rescind, or through a `git`
dependency on rescind's repository at a release tag; the traits take
`serde_json::Value` where the protocol does, so a hook needs `serde_json`
too:

```toml
[dependencies]
rescind-hook-sdk = { path = "../rescind/sdk/rust" }
serde_json = "1"
```
...
```text
$ cargo build -p rescind-hook-sdk --example audit_hook
$ printf '%s\n' '{"register":{"ok":true}}' \
