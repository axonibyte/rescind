...
printf '%s\n' '{"entry":{"seq":1},"id":1,"kind":"journal","op":"append"}' |
  RESCIND_AUDIT_LOG=/tmp/audit.ndjson sh examples/audit.sh
```
...
The README shows a session through `rescind-hook` itself: the registration
line, the acknowledgement, and one reply per request, each with the
request's id. This is how `rescindd` sees it. `tests/audit_example.rs` runs the
...
```text
$ rescind sdk-conform --name audit "rescind-hook --name audit --kinds journal,notify --command 'sh examples/audit.sh'"
...
RESCIND_AUDIT_LOG=$PWD/audit.ndjson rescindd run --dry-run --site site.scind \
  ...
  --spawn audit="rescind-hook --name audit --kinds journal,notify --command 'sh examples/audit.sh'"
```
...
`cargo test -p rescind-hook` runs the shim's tests: a POSIX `sh` script behind
it passing every conformance case (`tests/conform.rs`), and the quick start
(`tests/audit_example.rs`). They are ordinary workspace tests, so rescind's gate
