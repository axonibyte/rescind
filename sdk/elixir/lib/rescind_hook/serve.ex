defmodule RueHook.Serve do
  @moduledoc """
  The way `rescindd` reaches a hook it spawned (docs/hook-protocol.md, "A hook
  over stdio").
  ...
  @doc """
  Serve as a child the daemon spawned (`rescindd run --spawn`).
  ...
  def stdio(name, %Hooks{} = h, opts \\ []) do
    ...
    case IO.gets("") do
      :eof ->
        refuse("no acknowledgement from rescindd")
  ...
  defp refuse(why) do
    IO.puts(:stderr, "rescind_hook: " <> why)
    System.halt(1)
  end
end
